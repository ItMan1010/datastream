  ...
  <div class="main-content">
    <div v-show="false">
      <el-form ref="queryForm" :label-position="'left'" :inline="true" :model="queryForm">
        <el-form-item>
          <el-button class="ml-20" type="primary" :loading="loading">增加遮蔽罩</el-button>
        </el-form-item>
      </el-form>
    </div>
      <div class="pl-20 pr-20" style="padding-top: 20px;">
        <el-table :data="tableData" fit stripe highlight-current-row style="width: 100%;">
          ...
          <el-table-column prop="content" label="操作内容" width="200" :show-overflow-tooltip="true"></el-table-column>
          <el-table-column prop="userAgent" label="请求代理" :show-overflow-tooltip="true"></el-table-column>
        </el-table>
  ...
  methods: {
    ...
    queryLog() {
      ...
      http(constant.QUERY_SYSTEM_LOG, 'post', request).then(res => {
        this.loading = false
        if (res.errorCode !== '0') {
          this.$message.error(`查询登录日志失败：${res.errorMsg}`)
          return
        }
        ...
        this.loading = false
        this.$message.error(`查询登录日志失败：${err}`)
      })
    }
  }
}
