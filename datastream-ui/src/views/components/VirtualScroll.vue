...
export default {
  ...
  computed: {
    ...
    offsetMap () {
      ...
      for (let i = 0; i < this.data.length; i++) {
        const key = this.data[i][this.keyProp]
        res[key] = total

        const curSize = this.sizes[key]
        const size = typeof curSize === 'number' ? curSize : this.height
        total += size
      }
      return res
    }
  ...
  methods: {
    ...
    initData () {
      ...
      // 监听事件
      this.onScroll = this.throttle(this.handleScroll, this.throttleTime)
    ...
    updateSizes () {
      ...
      Array.from(rows).forEach((row, index) => {
        ...
        if (this.sizes[key] !== offsetHeight) {
          // Vue 3 兼容的响应式更新
          this.$set ? this.$set(this.sizes, key, offsetHeight) : (this.sizes[key] = offsetHeight)
        }
      })
    },
