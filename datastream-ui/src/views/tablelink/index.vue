...
<template>
  <div style="height: 100%;">
    <transition name="component-fade" mode="out-in">
      <div style="height: 100%;" :key="showBusiConfig ? 'config' : 'list'">
        <!-- 列表页面 -->
        <div v-if="!showBusiConfig" style="height: 100%;">
              ...
              <el-form ref="queryFormRef" :label-position="'left'" :inline="true" :model="queryForm">
                <el-row style="line-height: 30px;">
                  <el-col :span="18">
                    ...
                    <el-form-item>
                      <el-button class="ml-20" type="primary" :loading="loading" @click="queryTableLink">
                  ...
                  <el-col :span="6">
                    <el-button style="float: right; margin-right: 300px;" type="primary" plain @click="addFlowDefine">
              ...
              <el-table :data="tableData" fit stripe highlight-current-row style="width: 100%;">
                ...
                <el-table-column label="状态" width="80">
                  <template #default="scope">
                    ...
                    <el-tag v-if="scope.row.state === 0" type="info" effect="dark">删除</el-tag>
                  </template>
                </el-table-column>
                <el-table-column prop="stateDate" label="状态时间" width="160" />
                <el-table-column prop="flowDes" label="描述" />
                <el-table-column fixed="right" label="操作" width="380">
                  <template #default="scope">
                    <div class="datasource-opt">
                      <el-button type="primary" size="small" @click="showFlowDetail(scope.row)" plain>
                        <el-icon><Search /></el-icon>
                        详情
                      </el-button>
                      ...
                      <el-button type="warning" size="small" @click="modifyFlow(scope.row)" plain>
                        <el-icon><Edit /></el-icon>
                        修改
                      </el-button>
                      <el-button type="danger" size="small" @click="deleteTableLink(scope.row)" plain>
        ...
        <!-- 配置页面 -->
        <BusiConfig v-if="showBusiConfig" :mode="mode" :flow-detail="flowDetail" @go-back="goBack" />
      </div>
    </transition>
  </div>
</template>
...
import TableLinkTemplate from '@/views/components/TableLinkTemplate.vue'
import { useTableLinkManage } from '@/composables/useTableLinkManage'

export default {
  name: 'TableLinkConfig',
  components: {
    BusiConfig: TableLinkTemplate,
    Search,
    Plus,
    Switch,
    Edit,
  ...
  setup() {
    const tableLinkManage = useTableLinkManage()

    onMounted(() => {
      tableLinkManage.changeQueryFlag()
    })

    onActivated(() => {
      tableLinkManage.showBusiConfig.value = false
    })
