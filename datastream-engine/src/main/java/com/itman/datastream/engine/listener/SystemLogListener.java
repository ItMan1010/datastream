...
public class SystemLogListener implements ApplicationListener<SystemLogEvent> {
    ...
    public void onApplicationEvent(SystemLogEvent event) {
        try {
            SystemLogEntity canalSystemLog = new SystemLogEntity();
            BeanUtils.copyProperties(event.getSource(), canalSystemLog);
            SpringUtil.getBean(ISystemLogService.class).appendSystemLog(canalSystemLog);
